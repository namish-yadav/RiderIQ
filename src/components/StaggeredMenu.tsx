...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const [textLines, setTextLines] = useState<string[]>(['Navigation', 'Close Panel']);
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.4, ease: 'power3.out' }, i * 0.05);
    });
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    const currentLabel = opening ? 'Navigation' : 'Close Panel';
    const targetLabel = opening ? 'Close Panel' : 'Navigation';
  ...
  return (
    ...
    >
      {/* Backdrop overlay for dimming and click-away */}
      <div
      ...
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#090a0f', '#0e7490', '#06b6d4'];
